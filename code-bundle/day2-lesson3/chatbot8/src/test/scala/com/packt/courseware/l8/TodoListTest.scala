...
import com.packt.courseware.l8.modes.{ChatbotMode, TodoList}
...
class TodoListTest extends FunSuite {
  ...
  test("adding to emoty todolist") {
    ...
    val p1 = mode.process(s"todo new ${message}")
    assert(p1.isDefined)
    assert(p1.get.answer.startsWith("ok"))
    val p2 = p1.get.nextMode.process("todo list")
    assert(p2.isDefined)
  ...
  test("ambiguos") {
    var mode: ChatbotMode = TodoList.empty
    for(i <- 0 until 10) {
      val p = mode.process(s"todo new $i")
      mode = p.get.nextMode
    }
    val p2 = mode.process("todo done 5")
    assert(p2.isDefined)
